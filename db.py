...
async def unfreeze_to_worker(client_id: int, worker_id: int, total_usdt: float, amount_usdt: float = 0.0) -> bool:
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute("INSERT INTO bot_profit (amount) VALUES ($1)", bot_amount)
    return True
...
async def create_order_safe(uid: int, rub: float, total_usdt: float, amount_usdt: float, unique: bool) -> int:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT create_order_and_freeze($1, $2, $3, $4, $5)",
            uid, rub, total_usdt, amount_usdt, unique
        )
    return row["create_order_and_freeze"] if row else 0
